...
std::optional<STAT0> BQ21088::read_stat0() {
  uint8_t reg = 0;
  ...
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ...
    return std::optional<STAT0>();
  }
  return std::optional<STAT0>(STAT0(reg));
}

std::optional<STAT1> BQ21088::read_stat1() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0x1, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading STAT1 register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<STAT1>();
  }
  return std::optional<STAT1>(STAT1(reg));
}

std::optional<FLAG0> BQ21088::read_flag0() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0x2, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading FLAG0 register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<FLAG0>();
  }
  return std::optional<FLAG0>(FLAG0(reg));
}


std::optional<VBAT_CTRL> BQ21088::read_vbat_ctrl() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0x3, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading VBAT_CTRL register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<VBAT_CTRL>();
  }
  return std::optional<VBAT_CTRL>(VBAT_CTRL(reg));
}
...
std::optional<ICHG_CTRL> BQ21088::read_ichg_ctrl() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0x4, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading ICHG_CTRL register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<ICHG_CTRL>();
  }
  return std::optional<ICHG_CTRL>(ICHG_CTRL(reg));
}
...
std::optional<CHARGECTRL0> BQ21088::read_chargectl0() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0x5, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading CHARGECTRL0 register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<CHARGECTRL0>();
  }
  return std::optional<CHARGECTRL0>(CHARGECTRL0(reg));
}
...
std::optional<CHARGECTRL1> BQ21088::read_chargectl1() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0x6, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading CHARGECTRL1 register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<CHARGECTRL1>();
  }
  return std::optional<CHARGECTRL1>(CHARGECTRL1(reg));
}
...
std::optional<IC_CTRL> BQ21088::read_ic_ctrl() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0x7, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading IC_CTRL register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<IC_CTRL>();
  }
  return std::optional<IC_CTRL>(IC_CTRL(reg));
}
...
std::optional<TMR_ILIM> BQ21088::read_tmr_ilim() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0x8, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading TMR_ILIM register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<TMR_ILIM>();
  }
  return std::optional<TMR_ILIM>(TMR_ILIM(reg));
}
...
std::optional<SYS_REG> BQ21088::read_sys_reg() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0xA, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading SYS_REG register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<SYS_REG>();
  }
  return std::optional<SYS_REG>(SYS_REG(reg));
}
...
std::optional<MASK_ID> BQ21088::read_mask_id() {
  uint8_t reg = 0;
  esphome::i2c::ErrorCode err = this->read_register(0xC, &reg, 1);
  if (err != esphome::i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error reading MASK_ID register: %d", err);
    this->status_set_warning("I2C read error");
    return std::optional<MASK_ID>();
  }
  return std::optional<MASK_ID>(MASK_ID(reg));
}
...
void BQ21088::update() {
  ...
  if (stat0) {
    if (this->vin_pgood_sensor_ != nullptr) {
      this->vin_pgood_sensor_->publish_state(stat0->VIN_PGOOD_STAT);
      ESP_LOGD(TAG, "vin_pwr_good = %s", BooleanToString(stat0->VIN_PGOOD_STAT));
    } else {
      ESP_LOGD(TAG, "vin_pwr_good is null");
    }
  }
  #endif
  ...
  if (this->ichg_number_ != nullptr){
    this->ichg_number_->publish_state( icgh ? icgh->get_charge_current_mA()/1000.0f : NAN );
  }
  if (this->vbat_reg_number_ != nullptr) {
    auto vbat = this->read_vbat_ctrl();
    this->vbat_reg_number_->publish_state(vbat ? vbat->get_vbat_reg_voltage(): NAN);
  }
  if (this->buvlo_number_ != nullptr) {
    auto reg = this->read_chargectl1();
    this->buvlo_number_->publish_state( reg ? reg->buvlo_as_number() : NAN);
  }
  #endif
